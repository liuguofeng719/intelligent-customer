...
## 架构与数据流
前端 Vue 通过 REST + SSE 调用后端。后端编排流程为：FAQ 检索 → NL2DSL 业务查询 → SQL 执行 → 组合上下文提示词 → Ollama 生成 → SSE 流式返回。  
FAQ 文档通过导入接口切分与向量化后写入 Milvus。业务事实通过 NL2DSL 生成 SQL，执行后转为摘要注入提示词。

## 组件拆分
- 对话编排模块：负责意图识别、上下文拼装、LLM 调用。
- RAG 模块：FAQ 导入、切分、嵌入、检索。
- 业务数据模块：NL2DSL 解析、字段白名单校验、SQL 构建、查询与摘要生成。
- 模型接入模块：Ollama Chat 与 Embedding 客户端。
- API 控制层：对外 REST + SSE 接口。

## 接口契约（核心）
- `POST /api/chat`：输入 `question`，SSE 输出流式回复，并尝试注入业务摘要。
...
坚持标准化生态复用：Spring Boot + LangChain4j 官方依赖，MyBatis-Plus 数据访问层，Milvus 官方 SDK/客户端。

## 测试策略
仅覆盖核心闭环：
- 单元测试：提示词拼装、NL2DSL 解析、SQL 构建、业务摘要生成。
